        ...
        public void ConnectNearbyTemplates(PipeTemplate pipeTemplate, bool recursive = true)
        {
            ...
            foreach (PipePiece pipePiece in pipeTemplate.PipePieces)
            {
                ...
                foreach (int[] offset in indexOffsets)
                {
                    ...
                    try
                    {
                        checkingCell = _grid.Cells[
                            pipePiece.GridPosition[0] + offset[0],
                            pipePiece.GridPosition[1] + offset[1]];
                        ...
                        _pipeDividers.FirstOrDefault(divider =>
                        (divider.Connection[0].SequenceEqual(cell1) && divider.Connection[1].SequenceEqual(cell2)) ||
                        (divider.Connection[0].SequenceEqual(cell2) && divider.Connection[1].SequenceEqual(cell1)));

                        if (checkingCell != null)
                        {
                            if (checkingCell != pipeTemplate && checkingCell is PipeTemplate nearbyTemplate)
                            {
                                if (pipeTemplate.FuelType == FuelType.Any || nearbyTemplate.FuelType == FuelType.Any
